...
void GlobalMapperRos::GetParams()
{
  ...
  fla_utils::SafeGetParam(pnh_, "depth_max_x", params_.depth_max_x);
  fla_utils::SafeGetParam(pnh_, "depth_max_y", params_.depth_max_y);
  fla_utils::SafeGetParam(pnh_, "depth_max_z", params_.depth_max_z);
...
void GlobalMapperRos::InitSubscribers()
{
  ...
  lidar_sub_ = pnh_.subscribe("lidar_pointcloud_topic", 1, &GlobalMapperRos::LidarCallback, this);
}
...
void GlobalMapperRos::InitPublishers()
{
  if (publish_occupancy_grid_)
  {
    ...
    //NewPointCloud_ = pnh_.advertise<pcl::PointCloud<pcl::PointXYZ>>("NewPointCloudTopic", 1);
  }
                                                   ...
                                                   sensor_msgs::PointCloud2* pointcloud)
{
  ...
  int counter = 0;
  // std::cout << grid_dimensions[0] << ", " << grid_dimensions[1] << ", " << grid_dimensions[2] << std::endl;
...
void GlobalMapperRos::LidarCallback(const sensor_msgs::PointCloud2ConstPtr& lidar_ptr)
{
  ...
  {
    got_lidar_points_ = true;
  }

  // Convert from sensor_msgs/Pointclou2 to pcl::pointXYZ
  pcl::PCLPointCloud2* cloud = new pcl::PCLPointCloud2;
  pcl::PCLPointCloud2ConstPtr cloudPtr(cloud);
  pcl_conversions::toPCL(*lidar_ptr, *cloud);
  pcl::PointCloud<pcl::PointXYZ> cloudxyz;
  pcl::fromPCLPointCloud2(*cloud, cloudxyz);

  // variable to add intensity
  pcl::PointCloud<pcl::PointXYZI> cloudxyzi;

  // NewPointCloud_.publish(cloudxyz);
  for (int i = 0; i < cloudxyz.width; i = i + (params_.skip + 1))
  {
    pcl::PointXYZI point;
    // bool finite_x = isfinite(cloudxyz.points[i].x);
    bool finite_x = std::isfinite(cloudxyz.points[i].x);
    bool finite_y = std::isfinite(cloudxyz.points[i].y);
    bool finite_z = std::isfinite(cloudxyz.points[i].z);
    if(finite_x && finite_y && finite_z)
    {
      //std::cout << cloudxyz.points[i].x << "," << cloudxyz.points[i].y << "," << cloudxyz.points[i].z << std::endl;
      if ((std::fabs(cloudxyz.points[i].x)) > params_.depth_max_x || (std::fabs(cloudxyz.points[i].y)) > params_.depth_max_y || (std::fabs(cloudxyz.points[i].z)) > params_.depth_max_z)
      {
    ...
    {  // Nan and Inf
      // if (!finite_x)
      // {
        point.x = clear_unknown_distance_;
        point.y = clear_unknown_distance_;
        point.z = clear_unknown_distance_;
        point.intensity = nan(""); //cloudxyz.points[i].x;
        std::cout << "in NaN loop now" << std::endl;
      // }
      // if (!finite_y)
      // {
      //   point.x = clear_unknown_distance_;
      //   point.y = clear_unknown_distance_;
      //   point.z = clear_unknown_distance_;
      //   point.intensity = nan(""); //cloudxyz.points[i].y;
      // }
      // if (!finite_z)
      // {
      //   point.x = clear_unknown_distance_;
      //   point.y = clear_unknown_distance_;
      //   point.z = clear_unknown_distance_;
      //   point.intensity = nan(""); //cloudxyz.points[i].z;
      // }
    }
    cloudxyzi.push_back(point);
  }
  //std::cout << clear_unknown_distance_ << "," << params_.depth_max_z << std::endl;

  const std::string target_frame = params_.global_frame;
  geometry_msgs::TransformStamped transform_stamped;
  try
  {
    transform_stamped = tf_buffer_.lookupTransform(target_frame, lidar_ptr->header.frame_id,
                                                   ros::Time(lidar_ptr->header.stamp), ros::Duration(0.12));
  }
  catch (tf2::TransformException& ex)
  {
    ROS_WARN("[GlobalMapperRos::lidarCallback] %s", ex.what());
    return;
  }
  ...
  pcl::transformPointCloud(cloudxyzi, world_cloud, eigen_transform);
  ...
  tstampLastPclFused_ = (*lidar_ptr).header.stamp;
